...
@class AwesomeMenuManager;
@implementation UIImage (AwesomeMenu)

+ (nullable UIImage *)awesomeMenu_imageNamed:(NSString *)name{
    if(name &&
       ![name isEqualToString:@""]){
        NSBundle *bundle = [NSBundle bundleForClass:NSClassFromString(@"AwesomeMenuManager")];
        NSURL *url = [bundle URLForResource:@"AwesomeMenu" withExtension:@"bundle"];
...
+ (UIImage *)awesomeMenu_imageWithColor:(UIColor *)color {
    return [self awesomeMenu_imageWithColor:color size:CGSizeMake(1, 1)];
}
