...
@implementation AwesomeMenuUtil
- (instancetype)init{
    self = [super init];
    if (self) {
        _fileSize = 0;
...
+ (void)savePerformanceDataInFile:(NSString *)fileName data:(NSString *)data{
    ...
    NSString *anrDir = [cachesDir stringByAppendingPathComponent:@"AwesomeMenuPerformance"];
...
+ (void)clearLocalDatas {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (NSString *folder in folders) {
            [AwesomeMenuUtil clearFileWithPath:[homePath stringByAppendingPathComponent:folder]];
        }
    });
}

+ (void)openPlugin:(UIViewController *)vc {
    [AwesomeMenuHomeWindow openPlugin:vc];
}
...
+ (void)share:(id)object formVC:(UIViewController *)vc{
    ...
    if([AwesomeMenuAppInfoUtil isIpad]){
